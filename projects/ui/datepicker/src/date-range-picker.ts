...
@Component({
  ...
  template: `
    <div class="cal">
      <div class="head">
        <button type="button" class="nav" aria-label="Previous month" [disabled]="disabled()" (click)="shift(-1)">‹</button>
        <span class="month">{{ MONTHS[view()[1]] }} {{ view()[0] }}</span>
        <button type="button" class="nav" aria-label="Next month" [disabled]="disabled()" (click)="shift(1)">›</button>
      </div>
      ...
      <div class="grid" role="grid">
        ...
        @for (c of days(); track c.iso) {
          <button type="button" class="cell" [class.start]="c.iso === range().start" [class.end]="c.iso === range().end"
                  [class.inrange]="inRange(c.iso)" [disabled]="disabled()" (click)="pick(c.iso)" (mouseenter)="hover.set(c.iso)">{{ c.day }}</button>
        }
      </div>
